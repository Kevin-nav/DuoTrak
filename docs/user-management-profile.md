...
- `src/components/profile-content.tsx`:
    ...
    - **Mechanism:**
        ...
        - **Profile Header:** Shows avatar, username, email, and badges (streak, goals, partner status).
        ...
        - **Partner Connection:** Conditionally displays partner details.
...
### Contexts/Hooks/Libs
- `src/contexts/UserContext.tsx`: Provides the authenticated user's data to the profile page and other components, ensuring consistent user information across the application. Contains `UserDetails` interface with comprehensive user data.
...
4.  Notification settings and timezone updates are sent to `PATCH /api/v1/users/me` via `apiFetch`.
...
- `backend/app/api/v1/endpoints/users.py`:
    ...
    - **`PATCH /me` (response_model=UserRead):** Updates the authenticated user's profile. Takes `UserUpdate` schema and calls `user_service.update_user`.
...
- `backend/app/services/user_service.py`:
    ...
    - **Methods:**
        ...
        - `update_user(db, user_id, user_in)`: Updates specified fields of an existing user.
...
- Updating existing user records (e.g., email, timezone, notification settings, profile picture URL).
