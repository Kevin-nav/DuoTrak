...
export default function EnhancedActivityCard({ pendingVerifications = 1 }: EnhancedActivityCardProps) {
  ...
  return (
    <MouseGlowEffect glowColor="hsl(var(--primary))" intensity="high">
      <motion.div
        whileHover={{ scale: 1.01 }}
        onHoverStart={() => setShowUrgency(true)}
        onHoverEnd={() => setShowUrgency(false)}
        className="bg-primary/10 border border-primary/20 rounded-lg p-4 cursor-pointer"
      >
        <div className="flex items-center justify-between">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="flex items-center space-x-2 bg-primary hover:bg-primary/90 text-primary-foreground px-4 py-2 rounded-lg font-medium transition-all duration-200 text-sm relative overflow-hidden"
          >
        ...
        <AnimatePresence>
          {showUrgency && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="mt-3 flex items-center space-x-2 text-xs text-primary"
            >
              <motion.div
