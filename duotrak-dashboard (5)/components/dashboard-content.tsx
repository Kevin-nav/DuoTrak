...
export default function DashboardContent() {
  const { showWelcomeBack, showStreakCelebration, showPartnerActivity, showTaskCompletion, showMotivationalReminder } =
    useMascotInteractions()

  // Show welcome message when dashboard loads
  useEffect(() => {
    const timer = setTimeout(() => {
      showWelcomeBack("Alex")
    }, 2000)
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      showMotivationalReminder(timeOfDay)
    }, 8000)
  ...
  const handleGoalComplete = (goalId: number) => {
    ...
    if (stats.currentStreak === 7) {
      setTimeout(() => {
        showStreakCelebration(7, true)
      }, 3000)
    }
  }

  const handlePartnerActivity = () => {
    showPartnerActivity("Sarah")
  }
