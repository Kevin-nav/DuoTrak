...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 flex items-center justify-around bg-card border-t border-border h-16 lg:hidden">
        {navigation.map((item) => {
          const isActive = pathname === item.href
          const Icon = item.icon
          ...
          return (
            <Link key={item.name} href={item.href} className="flex-1">
              <motion.div
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className={`flex flex-col items-center justify-center py-1 transition-all text-xs ${
                  isActive
                    ? "text-primary"
